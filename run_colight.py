...
file_prefix = args.prefix +"_"+"Colight"+ "_" +datetime.now().strftime('%Y%m%d-%H%M%S')
    ...
    def train(self):
        ...
        for e in range(self.args.episodes):
            ...
            while i < self.args.steps:
                if i % self.args.action_interval == 0:
                    ...
                    if (total_decision_num > self.agent.learning_start):
                        actions = self.agent.get_actions(last_obs, test_phase=False)
                        # the retured dimension is [batch, agents],
                        # the batch is 1 when we get action, so we just get the first actions
                        # actions = actions[0]
                    else:
                        actions = self.agent._sample(s_size=self.agent.num_agents)
                    ...
                    self.agent.remember(last_obs, actions, rewards, obs)
            ...
            if self.args.test_when_train:
                result = self.train_test(e)
                if best_result is None or result < best_result:
                    best_result = result
                    self.agent.save_model(e, "{}/{}".format(self.args.save_dir, file_prefix))
                    print("best model saved! Result: {}, step:{}/{}".format(best_result, e, self.args.episodes))
        ...
        for i in range(self.args.test_steps):
            if i % args.action_interval == 0:
                ...
                for j in range(len(self.world.intersections)):
                    ...
                    node_dict = self.world.id2intersection[node_id_str]
                    last_phase.append(node_dict.current_phase)
                actions = self.agent.get_actions(obs,test_phase=True)
                # actions = actions[0]
                rewards_list = []
                for _ in range(self.args.action_interval):
        ...
        for i in range(self.args.test_steps):
            if i % args.action_interval == 0:
                ...
                if self.args.get_attention:
                    actions, att_step = self.agent.get_actions(obs,test_phase=True)
