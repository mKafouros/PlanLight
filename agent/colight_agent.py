...
class CoLightAgent(RLAgent):
    ...
    def get_actions(self, obs, test_phase=True):
        """
        phase : [agents]
        ...
        if not test_phase:
            ...
            if np.random.rand() <= self.epsilon:
                return self._sample([1,self.num_agents])[0]
        ob = obs[:, :-1]
        phase = obs[:, -1]
        e_ob = ob[np.newaxis, :]
        ...
        if self.get_attention:
            ...
            return np.argmax(act_values, axis=-1)[0], att_mat #[batch, agents],[batch,agent,nv,neighbor]
        else:
            act_values = self.sess.run(self.value,feed_dict=my_feed_dict)
            return np.argmax(act_values, axis=-1)[0] #batch, agents
    ...
    def _sample(self, s_size):
        return np.random.randint(0, self.action_space.n, s_size)

    def get_rewards(self, reward_type="vehicleNums"):
    ...
    def remember(self, ob, action, reward, next_ob):
        self.replay_buffer.append((ob, action, reward, next_ob))
    ...
    def replay(self):
        ...
        for i in range(len(minibatch)):
            ob = minibatch[i][0][:, :-1]
            phase = minibatch[i][0][:, -1]
            next_ob = minibatch[i][3][:, :-1]
            next_phase = minibatch[i][3][:, -1]
            obs.append(ob)
            phases.append(phase)
    ...
    def load_model(self, dir="model/bc", model_id=None):
        if model_id is None:
            print(dir)
            print(tf.train.latest_checkpoint(dir))
